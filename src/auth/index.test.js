...
import { AmplifySignIn } from 'auth/routes/SignIn'
import { AmplifySignUp } from 'auth/routes/SignUp'
import { AmplifyConfirmSignUp } from 'auth/routes/ConfirmSignUp'
import { AmplifyResendSignUp } from 'auth/routes/ResendSignUp'
import AuthApp from './'
...
describe('PublicRoute Component', () => {
    ...
    it('when store auth state is true, route \'/signup\' has no AmplifySignUp Component', () => {
        // Initialize mockstore with a state
        const initialState = { auth: { AUTHENTICATED: true } }
        const store = mockStore(initialState)
        const wrapper = mount(<Provider store={store}><MemoryRouter initialEntries={['/signup']}>
    ...
    it('when store auth state is false, route \'/confirmsignup\' has AmplifySignUp Component', () => {
        // Initialize mockstore with a state
        const initialState = { auth: { AUTHENTICATED: false } }
        const store = mockStore(initialState)
        const wrapper = mount(<Provider store={store}><MemoryRouter initialEntries={['/confirmsignup']}>
            <AuthApp />
        </MemoryRouter> </Provider>)

        expect(wrapper.find(AmplifyConfirmSignUp)).toHaveLength(1);
    })


    it('when store auth state is true, route /confirmsignup has no AmplifySignUp Component', () => {
        // Initialize mockstore with a state
        const initialState = { auth: { AUTHENTICATED: true } }
        const store = mockStore(initialState)
        const wrapper = mount(<Provider store={store}><MemoryRouter initialEntries={['/signup']}>
        ...
        expect(wrapper.find(AmplifyConfirmSignUp)).toHaveLength(0);
    })
    ...
    it('when store auth state is false, route /resendsignup has Resend Signup Component', () => {
        // Initialize mockstore with a state
        const initialState = { auth: { AUTHENTICATED: false } }
        const store = mockStore(initialState)
        const wrapper = mount(<Provider store={store}><MemoryRouter initialEntries={['/resendsignup']}>
            <AuthApp />
        </MemoryRouter> </Provider>)

        expect(wrapper.find(AmplifyResendSignUp)).toHaveLength(1);
    })


    it('when store auth state is true, route /resendsignup has no Resend Signup Component', () => {
        // Initialize mockstore with a state
        const initialState = { auth: { AUTHENTICATED: true } }
        const store = mockStore(initialState)
        const wrapper = mount(<Provider store={store}><MemoryRouter initialEntries={['/signup']}>
        ...
        expect(wrapper.find(AmplifyResendSignUp)).toHaveLength(0);
    })
